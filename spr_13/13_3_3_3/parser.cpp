...
namespace queries {
    ...
    public:
        ...
        ReadResult Process(const BudgetManager& budget) const override {
            ...
            double result = static_cast<double>(budget.ComputeSum(GetFrom(), GetTo()));
            return { result };
        }
    ...
    */
    ...
    public:
        ...
        void Process(BudgetManager& budget) const override { 
            budget.AddBulkOperation(GetFrom(), GetTo(), BulkTaxApplier{ 1.0 - tax_rate_ });
        }
        ...
        public:
            std::unique_ptr<Query> Construct(std::string_view config) const override {
                auto parts = Split(config, ' ');
                double tax_rate = std::stod(std::string(parts[2]))/100;
