...
public:
    ...
    double ComputeEarned(const Date date_from, const Date date_to);
    double ComputeSpended(const Date date_from, const Date date_to);
...
private:
    ...
    std::vector<DayInfo> days_earned_;
    std::vector<DayInfo> days_spended_;
};
