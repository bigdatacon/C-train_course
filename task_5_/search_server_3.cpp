...
std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(const std::execution::parallel_policy& policy, const std::string& raw_query, int document_id) const {
    ...
    if (std::any_of(policy, query.minus_words.begin(), query.minus_words.end(), [this, document_id](const std::string& word){return word_to_document_freqs_.at(word).count(document_id);})) {
        return { matched_words, documents_.at(document_id).status };
    }
    ...
    std::copy_if(
		policy, query.plus_words.begin(), query.plus_words.end(), matched_words.begin(), /*std::back_inserter(matched_words),*/
		[this, document_id](const std::string& word) {
			/*if (word_to_document_freqs_.count(word) == 0) {
				return;
			}*/
            /*if (word_to_document_freqs_.at(word).count(document_id) && !(word_to_document_freqs_.count(word) == 0) ) {
                return word;*/
                return word_to_document_freqs_.at(word).count(document_id) && !(word_to_document_freqs_.count(word) == 0);
            //}
		}
	);
	std::sort(matched_words.begin(), matched_words.end());
