...
public:
    ...
    void Withdraw(const VehiclePlate& car) {
        ...
        if (it != now_parked_.end()) {
            ...
            complete_parks_[car] = duration_time; // тут не в секундах 
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        ...
        for (auto& pair : now_parked_) {
            ...
            now_parked_[pair.first] =  Clock::now(); //обнуляю время для текущей машини что на стоянке
        ...
        //может тут другой контейнер возвращается , тк complete_parks_ содержит Duration а тут нужно вернуть контейнер который содержит int64_t
