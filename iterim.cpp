...
public:
    void AddBus(const string& bus, const vector<string>& stops) {
        ...
        // заполняю переменную для автобусов
        buses_to_stops_.emplace(bus, stops);

        // заполняю структура для остановок
        for (auto stop : stops) { stops_to_buses_.emplace(stop, bus); }
    ...
    BusesForStopResponse GetBusesForStop(const string& stop) const {
        ...
        for (auto /*[k, v] : */ el : allbusesresponse_.stop_for_buses /*buses_to_stops_*/) {

            if (/*stops.stop_for_buses.count(stop)!=0*/  IsInstanceVec_(el.second /*v*/, stop)) { buses_set.buses.push_back(el.first /*k*/); }
        }
