...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        //1. Указание в поисковом запросе более чем одного минуса перед словами, которых не должно быть в документах, например: кот --пушистый. В      // середине слов минусы разрешаются, например: иван-чай.
       // 2. Отсутствие в поисковом запросе текста после символа «минус», например кот -.
       // 3. Наличие спецсимволов — то есть символов с кодами в диапазоне от 0 до 31 включительно — в тексте документов и поискового запроса.
    ...
    */
    ...
    int GetDocumentId(int index) const {
        ...
        if (index< 0 || index > GetDocumentCount()) { return SearchServer::INVALID_DOCUMENT_ID; }
            ...
            map<int, DocumentData> ::const_iterator end = documents_.end();

            int counter = 0;
            for (map<int, DocumentData>::const_iterator it = documents_.begin(); it != end; ++it) {
                counter++;
