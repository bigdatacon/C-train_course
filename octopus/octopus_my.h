...
public:
    ...
    /*"Написанный код к сожалению вообще бессмысленен. Сначала вы зачем-то пытаетесь запихнуть число в вектор указателей, а потом берете адрес временного объекта и возвращаете его (амперсанд в этом положении является оператором взятия адреса).
Внутри класса у вас должен храниться только PtrVector<Tentacle>, добавление будет выглядеть как создание нового щупальца оператором new, инициализация ScopedPtr полученым указателем и push_back в вектор. ScopedPtr нужен для того, что бы избежать утечки памяти если push_back зафейлится. Ну а возврат ссылки - это просто разыменование последнего элемента в векторе"*/
