...
bool Cell::hasCircularDependency( Cell* cell) {
    ...
    for (Cell* dependent : cell->using_cells_) {
...
bool Cell::CheckCircularDependenciess( const Impl& new_impl) {
    ...
    for (/*const Cell& cell : cells*/  const auto& position : cells) {
        ...
        if (ref_cell->state == CellState::NotVisited && hasCircularDependency(ref_cell))
            return true;
    }
    return false;
}
...
/*
bool Cell::CheckCircularDependencies(const Impl& new_impl)  {
