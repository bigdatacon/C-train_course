...
namespace transport_catalogue {
	...
	void InputReaderJson::ReadInputJsonRequest() {
		...
		else if (json_array_render.AsMap().find("height") != json_array_render.AsMap().end()) {
			render_data_.height = json_array_render.AsMap().find("height")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("padding") != json_array_render.AsMap().end()) {
			render_data_.padding = json_array_render.AsMap().find("padding")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("line_width") != json_array_render.AsMap().end()) {
			render_data_.line_width = json_array_render.AsMap().find("line_width")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("stop_radius") != json_array_render.AsMap().end()) {
			render_data_.stop_radius = json_array_render.AsMap().find("stop_radius")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("bus_label_font_size") != json_array_render.AsMap().end()) {
			render_data_.bus_label_font_size = json_array_render.AsMap().find("bus_label_font_size")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("bus_label_offset") != json_array_render.AsMap().end()) {
		...
		else if (json_array_render.AsMap().find("stop_label_offset") != json_array_render.AsMap().end()) {
		...
		else if (json_array_render.AsMap().find("stop_label_font_size") != json_array_render.AsMap().end()) {
			render_data_.stop_label_font_size = json_array_render.AsMap().find("stop_label_font_size")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("underlayer_color") != json_array_render.AsMap().end()) {
		
			render_data_.underlayer_color.push_back(GetColor(json_array_render.AsMap().find("underlayer_color")->second));
		}

		else if (json_array_render.AsMap().find("underlayer_width") != json_array_render.AsMap().end()) {
			render_data_.underlayer_width = json_array_render.AsMap().find("underlayer_width")->second.AsDouble();
		}

		else if (json_array_render.AsMap().find("color_palette") != json_array_render.AsMap().end()) {
		...
		for (const auto& json_obj : json_array_render.AsMap()) {
			...
			if (json_obj.first == "width") {
				...
				render_data_.underlayer_color.push_back(GetColor(json_obj.second));
				}
