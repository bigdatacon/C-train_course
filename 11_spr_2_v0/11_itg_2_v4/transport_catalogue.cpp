...
namespace transport_catalogue {
	...
	AllBusInfoBusResponse TransportCatalogue::GetAllBusInfo(string_view bus)  const {
		...
		//cout << fb->bus_name << "type" << fb->type << endl;
		if (fb) {
			...
			if (stops_v.size() != 0) {
				...
				if (fb->type == "true"s) {
					//cout << "outerer" << fb->bus_name << "type" << fb->type << endl;
					...
					//cout<< "inner" << fb->bus_name << "type" << fb->type << endl;
	...
	std::deque<Bus> TransportCatalogue::GetBuses() { return buses_; };
	std::deque<Stop> TransportCatalogue::GetStops() { return stops_; };
}