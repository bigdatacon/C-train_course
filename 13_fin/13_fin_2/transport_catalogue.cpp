...
namespace transport_catalogue {
	...
	std::vector<size_t> TransportCatalogue::GetVertexIds(std::string stop_name) {
		...
		for (auto it = stops_distance_time_.begin(); it != stops_distance_time_.end(); it++) {
			...
			if (it->first.first->stop_name == stop_name) {
				// Индекс элемента найден
				std::size_t vertex_main = std::distance(stops_distance_time_.begin(), it)*2;
				std::size_t vertex_another = std::distance(stops_distance_time_.begin(), it) * 2+1;
				vec.push_back(vertex_main);
				vec.push_back(vertex_another);
				return vec;
			}
		}
