...
namespace transport_catalogue {
	...
	std::set<domain::Stop*, StopPointerComparer> TransportCatalogue::GetStopSet() {
		...
		{
			// Создание указателя и добавление его в std::set
	...
	/*
	std::set<domain::Stop*, StopPointerComparer> TransportCatalogue::GetStopSet() {
		std::set<domain::Stop*, StopPointerComparer> stops_set(stops_.begin(), stops_.end());
		return stops_set;
	}
	*/

	size_t TransportCatalogue::GetStopVertexIdByName(const std::set<domain::Stop*, StopPointerComparer>& stop_set, std::string stop_name) {
		...
		if (it != stop_set.end()) {
			...
			// индексы должны быть четными, т.к. номера вершин хранятся в формате 2n и 2n+1
			return index;
		}
		else {
			return 0;
