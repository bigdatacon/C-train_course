...
int main() {
	...
	graph::DirectedWeightedGraph<int> graph(2); // создаю граф с 2 вершинами
	//graph::AddKnots(graph, tc);
	//graph::Router<int> router(graph); 

	graph::ActivityProcessor activityprocessor(graph, tc);
	
	graph::Router<int> router = activityprocessor.ProcessRoute();
