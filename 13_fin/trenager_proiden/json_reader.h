		...
		void ManageOutputRequests(TransportCatalogue& tc, MapRenderer& mr, graph::ActivityProcessor<Weight>& actprocess, graph::Router<Weight>& router)
		{
			...
			for (const auto& el : out_req_) {
				...
				if (el.type == "Map"s) {
					...
					if (tc.CheckExistenceOfStopByName( el.from) && tc.CheckExistenceOfStopByName( el.to)) {


						size_t index_from = tc.GetStopVertexIdByName( el.from) * 2; // поменять функцию чтобы у себя внутри  каталога вызывала получение stop_set
						size_t index_to = tc.GetStopVertexIdByName( el.to) * 2;
						...
						// Одной из остановок не существует 
						int request_id = el.id;
						std::string error_message = "not found"s;
						json::Node final_route_description = json::Builder{}
							.StartDict()
