...
namespace transport_catalogue {
	...
	void TransportCatalogue::AddStopDistance(StopDistancesDescription distance) {
		if (distance.distances.size() != 0) {
			...
			for (auto el : stop_dist_main) {
				...
				stops_distance_time_.emplace(make_pair(main_stop_ptr, another_stop_ptr), distance/ bus_velocity_);
			}
		}
	}
	...
	//void TransportCatalogue::AddWaitTime(const double bus_wait_time) { bus_wait_time_ = bus_wait_time; }
	//void TransportCatalogue::AddVelocity(const double bus_velocity) { bus_velocity_ = bus_velocity; }
	...
	std::string TransportCatalogue::GetStopNameByVertexId(const std::set<domain::Stop, StopComparer>& stops_set,  size_t vertex_id) {
		// Получение третьего элемента
		auto it = stops_set.begin();
		std::advance(it, vertex_id); // Перемещаем итератор к третьему элементу
		if (it != stops_set.end()) {
			return it->stop_name;
		}
	...
	std::string TransportCatalogue::GetStopNameByEdgeId(size_t EdgeId) {
		...
		if (EdgeId == 1) { map_indx == 0; offset == 1; }
			...
			if (EdgeId % 2 == 0) { map_indx == EdgeId / 2; offset == 0; } // если ребро четное то это ожидание 
			else { map_indx == EdgeId / 2; offset == 1; } // если ребро не четное то это поездка 
		}

		auto it = stops_distance_time_.begin();
		std::advance(it, map_indx); // Перемещаем итератор к нужному индексу
	...
	std::set<domain::Stop, StopComparer> TransportCatalogue::GetStopSet() {
		std::set<domain::Stop, StopComparer> stops_set(stops_.begin(), stops_.end());
		return stops_set;
	}


	size_t TransportCatalogue::GetStopVertexIdByName(const std::set<domain::Stop, StopComparer>& stop_set, std::string stop_name) {
