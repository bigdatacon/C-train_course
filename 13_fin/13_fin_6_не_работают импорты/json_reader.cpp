...
namespace transport_catalogue {
	...
	void InputReaderJson::ManageOutputRequests(TransportCatalogue& tc, MapRenderer& mr, graph::Router<Weight>& router) {
		...
		for (const auto& el : out_req_) {
			...
			if (el.type == "Route"s) {
				//graph::DirectedWeightedGraph<int> graph(2);
				...
				//graph::AddKnots(graph, tc, stop_set); 
