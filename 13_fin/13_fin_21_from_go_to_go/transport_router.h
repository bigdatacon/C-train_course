	...
	public:
		...
		void AddStopsRoundTrip(std::deque<std::string_view> stops, std::set<domain::Stop, transport_catalogue::StopComparer> stop_set) {
			...
			for (auto it = stops.begin(); it != std::prev(stops.end()); ++it) {
				...
				if /* (it == std::prev(stops.end()))*/ (std::next(it) == std::prev(stops.end())) {  //-это значит дошел до предпоследней остановки маршрута 
					...
					Edge<double> edge3 = { num_vertex_2_wait, num_vertex2_go, tc.GetWaitTime() };  // добавляю ребро с ожиданием для последней остановки 
		...
		// Тут версия для некольцевого еду обычной вершине к обычной - все данные пропадают 
		/*
		void AddStopsNonRoundTrip(std::deque<std::string_view> stops, std::set<domain::Stop, transport_catalogue::StopComparer> stop_set) {
			...
			for (auto it = stops.begin(); it != std::prev(stops.end()); ++it) {
				...
				Edge<double> edge1 = { num_vertex_1_wait, num_vertex1_go, tc.GetWaitTime() };
				Edge<double> edge2 = { num_vertex1_go, num_vertex2_go, time_min_1_2 };
		...
		*/
		...
		// тут для некольцевого еду от обычной вершине к wait - данные пропадают для запроса с id 5 
		void AddStopsNonRoundTrip(std::deque<std::string_view> stops, std::set<domain::Stop, transport_catalogue::StopComparer> stop_set) {
