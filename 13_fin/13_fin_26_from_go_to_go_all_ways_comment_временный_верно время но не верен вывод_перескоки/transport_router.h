	...
	public:
		...
		void AddStopsOneDirection(std::deque<std::string_view> stops, std::set<domain::Stop, transport_catalogue::StopComparer> stop_set) {
			...
			for (auto it = stops.begin(); std::next(it) != stops.end(); ++it) {
				...
				if (std::next(it)== std::prev(stops.end())) { // проверяю что дошел до последней остановки и заканчиваю цикл 
					Edge<double> edge_mirror = { num_vertex_1_wait, num_vertex_next_wait ,  time_inner };
