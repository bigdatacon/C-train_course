		...
		*/
		...
		void AddStopsOneDirection(const std::deque<std::string_view>& stops, const std::string& bus_name) {
			...
			for (auto it = stops.begin(); std::next(it) != stops.end(); ++it) {
				...
				size_t num_vertex_1_wait = tc.GetStopVertexIdByName(stop_set_, *it) * 2;
				size_t num_vertex1_go = num_vertex_1_wait + 1;
				size_t num_vertex_next_wait = tc.GetStopVertexIdByName(stop_set_, *std::next(it)) * 2;
				...
				Edge<double>* edge_mirror = new Edge<double>{ num_vertex_1_wait, num_vertex1_go, tc.GetWaitTime() , *it, 0 };  // добавляю ребро зеркало для первой остановки 
				...
				*/
				...
				for (auto it_inner = std::next(it); std::next(it_inner) != stops.end(); ++it_inner) {
					...
					size_t num_vertex_inner_next_wait = tc.GetStopVertexIdByName(stop_set_, *std::next(it_inner)) * 2;
