...
namespace transport_catalogue {
	...
	std::string TransportCatalogue::GetStopNameByVertexId(const std::set<domain::Stop, StopComparer>& stops_set,  size_t vertex_id) {
		...
		*/ 
		...
		// реализация для 2 вершин разницей в 2 раза 
		...
		// реализация для 2 вершин с нецелой разнцей для ребра ждать (то есть для одной и той же остановки)
		/*size_t vertex_id_actual = 0;
		if (std::floor(vertex_id) != vertex_id) {
			vertex_id_actual = std::floor(vertex_id);
		}
		else {
			vertex_id_actual = vertex_id;
		}
		auto it = stops_set.begin();
		std::advance(it, vertex_id_actual);
		if (it != stops_set.end()) {
		...
		// реализация для 2 вершин с целой разнцей 40 000
		/*size_t vertex_id_actual = 0;
		if (vertex_id >= 40000) {
			vertex_id_actual = vertex_id-40000;
		}
		else {
			vertex_id_actual = vertex_id;
		}
	...
	//получить map у которого для каждоый пары остановок вместо дистанции время движения 
	...
	//получить индекс элемента в set остановок
