...
*/
    ...
    std::string MapRenderer::DrawRouteGetDoc(const TransportCatalogue& tc) {
        ...
        for (const auto& bus : buses) {
            ...
            for (int i = 0; i < static_cast<int>(current_stops.size()); i++) {
                ...
                if (i == 0) {
                    ...
                    route_font.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.bus_label_offset.first, render_data_.bus_label_offset.second)).SetFontSize(render_data_.bus_label_font_size)\
                        .SetFontFamily(rout_description.font).SetFontWeight(rout_description.font_weight).SetData(rout_description.text_data)\
                        .SetFillColor(render_data_.underlayer_color).SetStrokeColor(render_data_.underlayer_color).SetStrokeWidth(render_data_.underlayer_width).SetStrokeLineCap(StrokeLineCap::ROUND).SetStrokeLineJoin(StrokeLineJoin::ROUND);

                    route.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.bus_label_offset.first, render_data_.bus_label_offset.second)).SetFontSize(render_data_.bus_label_font_size)\
            ...
            if (!same_stations) {
                ...
                route_font_not_same.SetPosition(Point(screen_coord_last.x, screen_coord_last.y)).SetOffset(Point(render_data_.bus_label_offset.first, render_data_.bus_label_offset.second)).SetFontSize(render_data_.bus_label_font_size)\
                    .SetFontFamily(rout_description.font).SetFontWeight(rout_description.font_weight).SetData(rout_description.text_data)\
                    .SetFillColor(render_data_.underlayer_color).SetStrokeColor(render_data_.underlayer_color).SetStrokeWidth(render_data_.underlayer_width).SetStrokeLineCap(StrokeLineCap::ROUND).SetStrokeLineJoin(StrokeLineJoin::ROUND);

                route_not_same.SetPosition(Point(screen_coord_last.x, screen_coord_last.y)).SetOffset(Point(render_data_.bus_label_offset.first, render_data_.bus_label_offset.second)).SetFontSize(render_data_.bus_label_font_size)\
                    .SetFontFamily(rout_description.font).SetFontWeight(rout_description.font_weight).SetData(rout_description.text_data).SetFillColor(rout_description.route_color);
        ...
        for (auto i = stops_for_route.begin(); i != stops_for_route.end(); ++i)
        {
            ...
            stop_description_font.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.stop_label_offset.first, render_data_.stop_label_offset.second)).SetFontSize(render_data_.stop_label_font_size)\
                .SetFontFamily("Verdana").SetData(*i)\
                .SetFillColor(render_data_.underlayer_color).SetStrokeColor(render_data_.underlayer_color).SetStrokeWidth(render_data_.underlayer_width).SetStrokeLineCap(StrokeLineCap::ROUND).SetStrokeLineJoin(StrokeLineJoin::ROUND);


            stop_description.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.stop_label_offset.first, render_data_.stop_label_offset.second)).SetFontSize(render_data_.stop_label_font_size)\
