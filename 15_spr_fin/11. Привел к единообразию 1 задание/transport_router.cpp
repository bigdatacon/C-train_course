...
namespace graph {
		...
		std::optional<size_t> ActivityProcessor::GetValueByKey(std::string_view key) {
			...
			if (it != stop_to_vertex_.end()) {
				return it->second;  // ¬озвращаем значение, св€занное с ключом
			}
			else {
				return std::nullopt;  //  люч не найден
			}
		}
		...
		void ActivityProcessor::AddStopsOneDirection(const std::deque<std::string_view>& stops, const std::string& bus_name) {
			...
			for (auto it = stops.begin(); std::next(it) != stops.end(); ++it) {
				double sum_time = 0; // накапливаемое врем€ движени€ по ходу маршрута с каждой следующей остановкой сюда 
				...
				int distance_inner = tc.GetStopDistance(*stop_1, *stop_1_next); // рассто€ние от остановки 
				double time_inner = distance_inner / (tc.GetVelocity() * 1000 / 60) + sum_time;

				Edge<double>* edge_mirror = new Edge<double>{ num_vertex_1_wait, num_vertex1_go, tc.GetWaitTime() , std::string(*it), 0 };  // добавл€ю ребро зеркало дл€ первой остановки 
				Edge<double>* edge_go_wait = new Edge<double>{ num_vertex1_go, num_vertex_next_wait, time_inner, bus_name, 1 };  // добавл€ю ребро зеркало дл€ первой остановки 
				...
				for (auto it_inner = std::next(it); std::next(it_inner) != stops.end(); ++it_inner) {
					...
					int distance_1_2 = tc.GetStopDistance(*stop_inner, *stop_inner_next); // рассто€ние от остановки 


					double time_min_1_2 = distance_1_2 / (tc.GetVelocity() * 1000 / 60) + sum_time; // увиличиваю врем€ на то что уже было накоплено пока ехал до этой остановки 
					...
					sum_time = time_min_1_2; // присваиваю значение накопленного времени - текущее потраченное врем€ 
		...
		void ActivityProcessor::AddStopsNonRoundTrip(std::deque<std::string_view> stops, const std::string& bus_name) {  // “ут если передать stops по константной ссылке то ошибка в utility
			AddStopsOneDirection(stops, bus_name); // «аполн€ю в пр€мом направлении
			std::reverse(stops.begin(), stops.end()); // разворачи€ю списов остановок 
			AddStopsOneDirection(stops, bus_name); // «аполн€ю в обратном направлении
