...
void MapRenderer::DrawRoute(TransportCatalogue& tc) {
    ...
    for (auto bus : buses) {
        // добавить проверку что bus.stops.size()!-0;
        ...
        bool same_stations = CheckSameStations(current_stops);
        ...
        for (int i = 0; i < current_stops.size(); i++) {
            ...
            if (!same_stations) {
                if (i == current_stops.size() - 1) {
                    Text route_font_not_same;
                    Text route_not_same;

                    route_font_not_same.SetPosition(Point(rout_description.x_last, rout_description.y_last)).SetOffset(Point(render_data_.bus_label_offset[0], render_data_.bus_label_offset[1])).SetFontSize(render_data_.bus_label_font_size)\
                        .SetFontFamily(rout_description.font).SetFontWeight(rout_description.font_weight)\
                        .SetFillColor(render_data_.underlayer_color[0]).SetStrokeColor(render_data_.underlayer_color[0]).SetStrokeWidth(render_data_.underlayer_width).SetStrokeLineCap(StrokeLineCap::ROUND).SetStrokeLineJoin(StrokeLineJoin::ROUND);

                    route_not_same.SetPosition(Point(rout_description.x_last, rout_description.y_last)).SetOffset(Point(render_data_.bus_label_offset[0], render_data_.bus_label_offset[1])).SetFontSize(render_data_.bus_label_font_size)\
                        .SetFontFamily(rout_description.font).SetFontWeight(rout_description.font_weight).SetData(rout_description.text_data).SetFillColor(rout_description.route_color);
        ...
        //Заполняю данные по остановкам (круги и названия текст)
        deque<string*> stops_for_route = bus.stops;
        std::sort(stops_for_route.begin(), stops_for_route.end(), [](std::string* a, std::string* b) {
            return *a < *b;
            });
        for (int i = 0; i < stops_for_route.size(); i++) {
 
            Circle c;
            const Stop* one = tc.FindStop(*stops_for_route[i]);
            const svg::Point screen_coord = proj_one(one->coordinates);
            ...
            stop_description_font.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.stop_label_offset[0], render_data_.stop_label_offset[1])).SetFontSize(render_data_.stop_label_font_size)\
                .SetFontFamily("Verdana").SetData(*stops_for_route[i])\
                .SetFillColor(render_data_.underlayer_color[0]).SetStrokeColor(render_data_.underlayer_color[0]).SetStrokeWidth(render_data_.underlayer_width).SetStrokeLineCap(StrokeLineCap::ROUND).SetStrokeLineJoin(StrokeLineJoin::ROUND);
            ...
            stop_description.SetPosition(Point(screen_coord.x, screen_coord.y)).SetOffset(Point(render_data_.stop_label_offset[0], render_data_.stop_label_offset[1])).SetFontSize(render_data_.stop_label_font_size)\
                .SetFontFamily("Verdana").SetData(*stops_for_route[i]).SetFillColor("black");
