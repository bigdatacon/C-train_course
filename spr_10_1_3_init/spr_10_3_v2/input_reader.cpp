...
unordered_set<pair<string, int>> SplitStringByForDist(string distance_section) {
	...
	for (string str : str_set) {
		...
		str = str.substr(first_space_colon, last_space_colon);
		string dist_s = str.substr(0, is_distance);
		int dist = std::stoi(str.substr(0, is_distance));
		size_t pos_sec_space = str.find(" ", str.find(" ") + 1);
		string stop_name = str.substr(pos_sec_space);
		res.insert(pair(stop_name, dist));
	}
	return res;
}
...
void InputReader::FillRequests() {
	for (int i = 0; i < num_update_q_; ++i) {
		...
		auto space_colon = line.find(" "); // поменять на request_section
		...
		if (req_name == "Bus"s) {
			...
			if (pos_sec_comma != std::string::npos) {
				unordered_set<std::pair<string, int>> stop_dist_updt;
				string cordinate_section = list_section.substr(0, pos_sec_comma);
...
void InputReader::GetUpdStopDist(TransportCatalogue& tc) {
	for (int i = 0; i < upd_req_stop_.size(); ++i) {
		//cout << "UPDT : " << upd_req_stop_[i].stop << endl;
