...
namespace transport_catalogue {
	...
	void TransportCatalogue::AddStop(Stop_Input stop) {
		Stop stp;
		stp.stop_name = stop.stop_name;
		stp.coordinates = stop.coordinates;
		stops_.push_back(stp);
	...
	/*void TransportCatalogue::AddStop(Stop stop) {
		Stop* ptr_s_first = &stop;
		if (ptr_s_first) {
			stops_.push_back(move(*ptr_s_first));
			Stop* ptr_stop = &stops_.back();
			stop_name_to_stop_.emplace(stop.stop_name, ptr_stop);
		}
	}*/
	...
	/*const Stop* TransportCatalogue::FindStop(const std::string_view* stop) const {
		...
		if (stop_name_to_stop_.count(stop_s)) { return static_cast<const Stop*>(stop_name_to_stop_.at(stop_s)); }
		else { return res; }
	}*/
