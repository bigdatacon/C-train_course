	...
	if (stops_v.size() != 0) {
		...
		if (fb.type == "c"s) {
			...
			for (int i = 0; i < stops_v.size() - 1; i++) {
				/*Stop one = FindStop(*stops_v[i]);
				Stop two = FindStop(*stops_v[i + 1]);
				coord_length += ComputeDistance(one.coordinates, two.coordinates);  // передалать чтобы FindStop возвращал указатель
				Stop* ptr_one = &one;
				Stop* ptr_two = &two;
				real_length += GetStopDistance(make_pair(ptr_one, ptr_two));*/
...
void TransportCatalogue::AddStopDistance(Stop s) {
	...
	unordered_set<pair<string, int>> stop_dist_main = s.stop_dist;
