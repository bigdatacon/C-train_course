...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings) {
	...
	for (const std::string& word : words) {
		...
		words_with_ids_[document_id].insert(word);  // добавляю id : слова 
	}
...
void RemoveDuplicates(SearchServer& search_server) {
	...
	for (int i = 0; i < (search_server.end() - search_server.begin()); ++i) {
		for (int j = i + 1; j < (search_server.end() - search_server.begin()); ++j) {
			if (words_with_ids_[i] == words_with_ids_[j]) {
				documents_to_remove.insert(j);
			}
		}
	}

	for (auto i : documents_to_remove) {
